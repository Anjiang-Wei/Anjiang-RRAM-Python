...
public:
  ...
  static vector<float> mutate_vec_int(vector<int> input, int R, int M, bool sign,
                                      int m_p, int m_a, float spec_ber, float raw_ber);
};
...
Rfloat::Rfloat(uint8_t R_, uint8_t M_, bool sign_, uint8_t content_[maxM]) {
...
Rfloat::Rfloat(uint8_t R_, uint8_t M_, bool sign_, int val) {
    R = R_; M = M_; sign = sign_;
    //Todo: implement this
}
...
static vector<int> mutate_vec_int(vector<int> input, int R, int M, bool sign,
                                  int m_p, int m_a, float spec_ber, float raw_ber);
    int size = input.size();
